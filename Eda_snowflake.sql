...
SELECT BANK,ACCOUNT_ID,LAST_TXN_DATE,count(*),MAX(CLOSING_BALANCE)AS CLOSING_BALANCE FROM CTE_1 GROUP BY 1,2,3 ORDER BY 1,2,3);
...
LEFT OUTER JOIN LOAN_KPI C ON A.BANK = C.BANK AND A.ACCOUNT_ID = C.ACCOUNT_ID);
